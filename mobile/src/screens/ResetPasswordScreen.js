...
export default function ResetPasswordScreen({ navigation }) {
  ...
  const updatePassword = async () => {
    ...
    try {
      ...
      if (!sessionData.session) throw new Error('Your reset session has expired. Request a new reset link.');
      const { error: updateError } = await supabase.auth.updateUser({ password });
      if (updateError) throw updateError;
      await signOut({ redirectToLogin: true });
    } catch (err) { setError(err?.message || authError(err)); }
    finally { lock.current = false; setBusy(false); }
  };

  return <AuthPage title="Reset password">
    {!authConfigured && <Text style={s.error}>Supabase configuration is required.</Text>}
    {!recoveryMode ? <Text style={s.note}>Open the password reset link from your email on this device to continue.</Text> : <>
