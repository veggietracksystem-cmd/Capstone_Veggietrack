...
[delivery_reject.sql](backend/sql/delivery_reject.sql) has also been applied: the September 20 read-only inspection found `deliveries.rejection_reason` and `deliveries.rejected_at` both **present**, so a rider's rejection reason is stored. No migration is currently outstanding.
